// app/src/main/java/dev/jdtech/jellyfin/viewmodels/MovieViewModel.kt
// 🔧 REMOVED TMDB & LOADING STATE - Chỉ sử dụng trailer từ Jellyfin, không loading state
...
constructor(
    ...
    private val _trailerState = MutableStateFlow<TrailerState>(TrailerState.None)
    ...
    // 🚫 REMOVED Loading state - Không cần loading state cho trailer
    sealed class TrailerState {
        data object None : TrailerState()
        data class Ready(val embedUrl: String) : TrailerState()
        data class Error(val error: Exception) : TrailerState()
    }
    ...
    fun loadData(itemId: UUID) {
        viewModelScope.launch {
            ...
            try {
                ...
                // Load trailer từ Jellyfin server ngay lập tức, không loading state
                loadTrailerFromJellyfin()
    ...
    private fun loadTrailerFromJellyfin() {
        viewModelScope.launch {
            try {
                // 🚫 REMOVED: Không emit Loading state nữa
                // _trailerState.emit(TrailerState.Loading)

                // Chỉ sử dụng trailer từ Jellyfin server - load ngay lập tức
                if (!item.trailer.isNullOrEmpty()) {
                    Timber.d("🎬 Using Jellyfin trailer: ${item.trailer}")
                    val embedUrl = convertToEmbedUrl(item.trailer!!)
                    _trailerState.emit(TrailerState.Ready(embedUrl))
                } else {
                    Timber.d("🎬 No trailer available from Jellyfin for: ${item.name}")
                    _trailerState.emit(TrailerState.None)
                }
            } catch (e: Exception) {
                Timber.e(e, "🎬 Error loading trailer from Jellyfin")
                _trailerState.emit(TrailerState.Error(e))
            }
        }
    }

    private fun convertToEmbedUrl(trailerUrl: String): String {
        return when {
            trailerUrl.contains("youtube.com/watch?v=") -> {
                val videoId = trailerUrl.substringAfter("v=").substringBefore("&")
            ...
            trailerUrl.contains("youtube.com/embed/") -> trailerUrl
            else -> {
                // Nếu không phải YouTube URL, có thể là URL trực tiếp từ Jellyfin
                Timber.d("🎬 Direct trailer URL from Jellyfin: $trailerUrl")
                trailerUrl
            }
        }
    }
