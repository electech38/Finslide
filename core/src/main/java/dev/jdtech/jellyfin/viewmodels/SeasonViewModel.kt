...
constructor(
    ...
    sealed class UiState {
        data class Normal(val episodes: List<EpisodeItem>, val season: FindroidSeason?) : UiState()
        data object Loading : UiState()
        data class Error(val error: Exception) : UiState()
    }

    private var season: FindroidSeason? = null
    ...
    fun loadEpisodes(seriesId: UUID, seasonId: UUID, offline: Boolean) {
        viewModelScope.launch {
            ...
            try {
                ...
                _uiState.emit(UiState.Normal(episodes, season))
    ...
    private suspend fun getEpisodes(seriesId: UUID, seasonId: UUID, offline: Boolean): List<EpisodeItem> {
        val header = EpisodeItem.Header(
            seriesId = season?.seriesId ?: seriesId,
            seasonId = season?.id ?: seasonId,
            seriesName = season?.seriesName.orEmpty(),
